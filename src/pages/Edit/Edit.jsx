...
import { editCocktail, getCocktail } from '../../services/cocktails.service';
...
export const Edit = () => {
  ...
  useEffect(() => {
    const fetchCocktailData = async () => {
      try {
        const { cocktail } = await getCocktail(cocktailId); // Fetch cocktail data
        const formattedData = {
          name: cocktail.name || '',
          description: cocktail.description || '',
          img: cocktail.img || '',
          ingredients: Array.isArray(cocktail.ingredients) ? cocktail.ingredients : [['', '']],
          recipe: Array.isArray(cocktail.recipe) ? cocktail.recipe : [''],
          taste: cocktail.taste || '',
          time: cocktail.time || '',
          level: cocktail.level || '',
        };

        setFormData(formattedData); // Populate form with fetched data
      } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!validationMessage) {
      try {
        await editCocktail(cocktailId, formData); // Use editCocktail for submitting
        navigate('/cocktails');
      } catch (error) {
